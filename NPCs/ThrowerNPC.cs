...
namespace TheThrowingMod.NPCs            //We need this to basically indicate the folder where it is to be read from, so you the texture will load correctly
{
    [AutoloadHead]
    public class ThrowerNPC : ModNPC
    {
        ...
        {
            button = "Buy Throwing";   //this defines the buy button name
        }
        ...
        public override void SetupShop(Chest shop, ref int nextSlot)       //Allows you to add items to this town NPC's shop. Add an item by setting the defaults of shop.item[nextSlot] then incrementing nextSlot.
        {
            if (NPC.downedBoss1)   //this make so when the king slime is killed the town npc will sell this
            {
                ...
                nextSlot++;
                shop.item[nextSlot].SetDefaults(ItemID.Javelin);  //an example of how to add a vanilla terraria item
                nextSlot++;
                shop.item[nextSlot].SetDefaults(ItemID.WormholePotion);
                nextSlot++;
                shop.item[nextSlot].SetDefaults(ItemID.IronskinPotion);
                nextSlot++;
                shop.item[nextSlot].SetDefaults(mod.ItemType("JavelinQuiver"));  //this is an example of how to add a modded item
                nextSlot++;
                shop.item[nextSlot].SetDefaults(mod.ItemType("ShinyRock"));  //this is an example of how to add a modded item
                nextSlot++;
            }
            ...
            shop.item[nextSlot].SetDefaults(mod.ItemType("AttunedBone"));  //this is an example of how to add a modded item
            nextSlot++;
            shop.item[nextSlot].SetDefaults(mod.ItemType("AttunedFossil"));  //this is an example of how to add a modded item
            nextSlot++;
        ...
        {
            int otherNPC = NPC.FindFirstNPC(NPCID.Angler);   //this make so when this npc is close to Wizard
            if (otherNPC >= 0 && Main.rand.Next(4) == 0)    //has 1 in 3 chance to show this message
            {
                return "Yes " + Main.npc[otherNPC].GivenName + " is a wizard.";
            }
        ...
        {
            scale = 1f;
            item = Main.itemTexture[mod.ItemType("ThrowiumJavelin")]; //this defines the item that this npc will use
            itemSize = 56;
        }
